...
public class BookingController {
    ...
    @GetMapping("/services/{service}/available-slots")
    public List<AvailableSlotResponse> availableSlots(
            @PathVariable("service") UUID serviceId,
            @RequestParam Instant from,
            @RequestParam Instant to) {
        return service.availableSlots(serviceId, from, to).stream()
                .map(slot -> new AvailableSlotResponse(
                        slot.startAt(),
                        slot.endAt(),
                        slot.availableStaff().stream().map(mapper::staff).toList()))
                .toList();
    }
